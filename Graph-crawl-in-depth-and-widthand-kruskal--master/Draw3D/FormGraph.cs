...
namespace Draw3D
{
    public partial class FormGraph : Form
    {
        ...
        private void поискВГлубинуToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                int n = Lib.graph.DepthSearch(Lib.NumNode, Graph.Nodes[Lib.NumNode].name);
                //Caption:=IntToStr(n);
                MyDraw(false);
            }
        ...
        private void поискВГлубинустекToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Program.formMyDialog.ShowDialog() == DialogResult.OK)
            {
                int n = Lib.graph.DepthSearchStack(0, Graph.Nodes[Lib.NumNode].name);
                //Caption:=IntToStr(n);
        ...
        private void поискВШиринуToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (Program.formMyDialog.ShowDialog() == DialogResult.OK)
            {
                int n = Lib.graph.BreadthSearch(Lib.NumNode, "------");//Graph.Nodes[Lib.NumNode].name);
                //Caption:=IntToStr(n);
                Text = "Path: " + Lib.myStack.StackToStr();
                MyDraw(false);
            }
        ...
        {
            Program.formMyDialog.Text = "Минимальное расстояние";
            if (Program.formMyDialog.ShowDialog() == DialogResult.OK)
            {
                int n = Lib.graph.Dijkst(0, Lib.NumNode);
                MyDraw(false);
        ...
        private void выделениеСвязностиToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            MyDraw(false);
            //MessageBox.Show("Path: " + Graph.StackToStr(Lib.myStack));
            int n = Graph.StackToStrCount(Lib.myStack);
            string s="";
            if (n == Graph.Nodes.Length)
                s = "Граф связен";
            else
                s = "Граф не связен";
            MessageBox.Show(s);
        ...
        private void симметрияToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Lib.graph.SetSim();
            MyDraw(false);
        }

        private void полныйГрафToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Lib.graph.SetFull();
            MyDraw(false);
        }

        private void генетическийToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Lib.graph.Генетический();
        }
    }
}
